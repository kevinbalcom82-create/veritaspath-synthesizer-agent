  ...
  try {
    ...
    if (!full_email_thread || typeof full_email_thread !== 'string' || full_email_thread.trim().length === 0) {
      return res.status(400).json({
        status: 'error',
        message: 'Invalid request body. Expected: {"full_email_thread": "..."}'
      });
    }
...
Sentiment: [Positive/Neutral/Urgent]`;
    ...
    const fullAnalysis = geminiResponse.text();
...
${fullAnalysis}`;
    ...
    const executiveDraft = claudeMessage.content
    ...
    console.error('Error in synthesize-email function:', error);
